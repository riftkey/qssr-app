...
const CarbonCalculator = () => {
  ...
  useEffect(() => {
    const fetchScope1 = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/emissions/scope1-summary/${selectedYear}`
        );
        if (res.ok) {
    ...
    const fetchScope2 = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/emissions/scope2-summary/${selectedYear}`
        );
        if (res.ok) {
          const data = await res.json();
          setScope2Data({
      ...
      try {
        ...
        const resultsScope1 = await Promise.all(
          campuses.map(async (campus) => {
            const res = await fetch(
              `http://localhost:5000/api/emissions/summary-campus/${campus}/${selectedYear}`
            );
            const data = await res.json();
            return {
