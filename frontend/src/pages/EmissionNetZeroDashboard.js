...
ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend);
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        // baseline
        const resBaseline = await fetch(`http://localhost:5000/api/emission-baseline/${campus}`);
        if (resBaseline.ok) {
        ...
        // trend
        const resTrend = await fetch(`http://localhost:5000/api/emission-trend/${campus}`);
        if (resTrend.ok) {
