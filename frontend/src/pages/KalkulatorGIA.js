...
const GIACalculator = () => {
  ...
  const handleSave = async () => {
    try {
      await fetch(
        `http://localhost:5000/api/gia/${selectedCampus}/${selectedYear}`,
        {
          method: "POST",
      ...
      alert("Data GIA berhasil disimpan");
    } catch (err) {
      console.error(err);
      alert("Gagal menyimpan data GIA.");
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Kalkulator Gross Internal Area (GIA)</h1>
      <button
  onClick={() => {
    introJs().setOptions({
      steps: [
        ...
        {
          element: ".tombol-tambah-gedung",
          intro: "Klik ini untuk menambahkan entri gedung baru.",
        },
        {
...
>
   Panduan Pengisian GIA
</button>


      {/* Kampus + Tahun */}
      <div className="flex space-x-2 mb-4 dropdown-tahun-kampus">
        <button
      ...
      <div className="mb-4 p-3 bg-gray-100 rounded shadow text-sm summary-gia">
        Total GIA kampus <strong>{selectedCampus}</strong> tahun {selectedYear}:{" "}
        <strong className="text-green-700">{totalLuas.toFixed(2)} m²</strong>
      </div>

      {data.map((row, i) => (
        <div
          key={i}
          className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-4 items-end input-gedung"
        >
          <div className="md:col-span-4">
            <label className="text-sm block mb-1">Nama Gedung</label>
            <input
              type="text"
              value={row.gedung}
              onChange={(e) => handleChange(i, "gedung", e.target.value)}
              className="border px-2 py-1 rounded w-full text-sm"
            />
          </div>
          <div>
            <label className="text-sm block mb-1">Luas Internal (m²)</label>
            <input
              type="number"
              value={row.luas}
              onChange={(e) => handleChange(i, "luas", e.target.value)}
              className="border px-2 py-1 rounded w-full text-sm"
            />
          </div>
      ...
      <button
        className="bg-blue-600 text-white px-4 py-2 rounded mt-2 tombol-tambah-gedung"
        onClick={() =>
          setData([...data, { gedung: "", luas: "" }])
        }
      >
        + Tambah Gedung
      </button>
      ...
      <button
        className="ml-4 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded mt-2 tombol-simpan-gia"
        onClick={handleSave}
      >
        Simpan Data
      </button>
      ...
      {modal.show && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg">
            <p className="text-lg mb-4">Hapus gedung ini?</p>
