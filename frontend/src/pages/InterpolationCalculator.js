// src/pages/InterpolationCalculator.js
...
const InterpolationCalculator = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`https://qssr-app-production.up.railway.app/api/emission-baseline/${campus}`);
  ...
  useEffect(() => {
    if (
      ...
      const progressRatio = (selectedYear - baselineYear) / yearDiffTarget;
      if (progressRatio < 0.05) {
        setWarning("Progress too low (<5%)");
      } else {
        setWarning("");
      }
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-xl font-bold mb-4">Kalkulator Interpolasi Emisi</h1>
      ...
      {expectedEmission !== null && !warning && (
        <div className="p-4 bg-green-100 rounded shadow text-lg">
